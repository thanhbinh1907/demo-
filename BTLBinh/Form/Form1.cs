...
namespace BTLBinh
{
    public partial class Form1 : Form
    {
        ...
        // Phương thức để xóa các TextBox không bao gồm txtMaSp
        private void ClearOtherTextBoxes()
        {
            foreach (var tb in new List<TextBox> { txtTenSp, txtMaLoai, txtGiaNhap, txtGiaBan, txtSoLuong, txtMaCD })
            {
                tb.Clear();
            }
        }
            ...
            else
            {
                ...
                if (textBoxes.All(tb => !string.IsNullOrWhiteSpace(tb.Text)))
                {
                    ...
                    if (function.CheckExists("SANPHAM", "MaSP", maSp))
                    {
                        ...
                        ClearTextBoxes(); // Gọi để xóa tất cả các TextBox nếu mã đã tồn tại
                    ...
                    // Xóa tất cả các TextBox sau khi thêm thành công
                    ClearTextBoxes();

...
namespace BTLBinh
{
    public partial class Form8 : Form
    {
        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            // Kiểm tra tài khoản từ bảng TaiKhoan
            DataProcess dataProcess = new DataProcess();
            string query = "SELECT MaNV FROM TaiKhoan WHERE MaNV = @MaNV AND MatKhau = @MatKhau";
            ...
            using (SqlConnection connection = new SqlConnection(dataProcess.ConnectionString))
            {
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@MatKhau", password); // Sử dụng MatKhau thay vì SDT
                    ...
                    if (reader.Read())
                    {
                        ...
                        User.SetSession(employeeId, employeeId, "Employee"); // Bạn có thể thay đổi để lấy tên nhân viên từ bảng NHANVIEN nếu cần
                        User.UpdateUserLabel(); // Cập nhật label
